...
def show_trends(performances: list[Performance]) -> None:
    """TODO: docstring"""
    st.header("Trends")
    timeline_agg = st.selectbox("Group by:", options=["Day", "Week", "Month", "Year"], index=3)
    hide_gaps = st.checkbox("Hide gaps")
    timeline_df = generate_timeline(performances, timeline_agg, hide_gaps)
